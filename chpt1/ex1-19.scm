;; There is a clever algorithm for computing Fibonacci numbers in a logarithmic
...
;; special case of p = 0 and q = 1 in a family of transformations T(pq), where
;; T(pq) transforms the pair (a,b) according to a <- bq + aq + ap and
;; b <- bp + aq. Show that if we apply such a transformation T(pq) twice, the
